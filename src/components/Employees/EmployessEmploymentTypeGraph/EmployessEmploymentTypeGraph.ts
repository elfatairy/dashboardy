...
export class EmployessEmploymentTypeGraph {
  ...
  private horizontalBarGraph: HorizontalBarGraph | null = new HorizontalBarGraph();
  ...
  private renderGraph(): void {
    ...
    const data = Object.values(employees.reduce((acc, employee) => {
      ...
      acc[employmentType].value++;
      return acc;
    }, {} as Record<string, { label: string; value: number }>)).sort((a, b) => b.value - a.value);
    

    this.horizontalBarGraph?.render(this.graph, {
      data,
      colors: [
        "#3498db",
        "#8e44ad",
