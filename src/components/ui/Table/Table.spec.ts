  ...
  renderRow: (row: Task) => {
    ...
    tr.innerHTML = `
      <td>${row.id}</td>
        ...
        <td><span>${row.currentStatus}</span></td>
        <td><span>${row.priority}</span></td>
        <td>
          <div>
            <div data-progress="${row.progress}"></div>
            <span>${row.progress}%</span>
          </div>
        </td>
        <td>${row.employees
          .map((employeeId) => getEmployeeById(employeeId)?.name)
          .join(", ")}</td>
    `;

    const progressBar = new ProgressBar();
    progressBar.render(tr.querySelector(`[data-progress]`) as HTMLElement, {
    ...
    searchHandler: (value: string, allRows: Task[]) => {
      return allRows.filter((row) => {
        return (
          ...
          getProjectById(row.projectId)
            ?.name.toLowerCase()
            .includes(value.toLowerCase()) ||
          row.employees
            .map((employeeId) =>
              getEmployeeById(employeeId)?.name.toLowerCase()
            )
            .join(", ")
            .includes(value.toLowerCase())
        );
    ...
    sortHandler: (option: string, allRows: Task[]) => {
      return allRows.sort((a, b) => {
        if (option === "id") {
          ...
          return (
            getProjectById(a.projectId)?.name.localeCompare(
              getProjectById(b.projectId)?.name || ""
            ) ?? 0
          );
...
describe("Table", () => {
  ...
  beforeAll(() => {
    ...
    container.dataset.testid = "test-table";
  ...
  test("should render", async () => {
    ...
    table.render(container, tableProps);
